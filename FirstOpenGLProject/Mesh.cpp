...
//old mesh stuff
/*Mesh::Mesh(std::vector<float> vertices, int faces) : m_vertices(vertices), m_faces(faces)
{
	innitBuffers();
}
void Mesh::innitBuffers()
{
...
{
	glBindVertexArray(m_VAO);
	glDrawArrays(GL_TRIANGLES, 0, 6 * m_faces);
	glBindVertexArray(0);
}*/
...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<unsigned int> indices, std::vector<Texture> textures) : m_vertices_(vertices), m_indices(indices), m_textures(textures)
{
	innitMesh();
}
...
void Mesh::innitMesh() 
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * m_vertices_.size(), m_vertices_.data(), GL_STATIC_DRAW);
